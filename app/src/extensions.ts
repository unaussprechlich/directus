...
const onHydrateFns: (() => Promise<void>)[] = [];
const onDehydrateFns: (() => Promise<void>)[] = [];
...
export function registerExtensions(app: App): void {
	...
	onHydrateFns.push(onHydrateModules);
	onDehydrateFns.push(onDehydrateModules);
}

export async function onHydrateExtensions() {
	await Promise.all(onHydrateFns.map((onHydrate) => onHydrate()));
}

export async function onDehydrateExtensions() {
	await Promise.all(onDehydrateFns.map((onDehydrate) => onDehydrate()));
}
